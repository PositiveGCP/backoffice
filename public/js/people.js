...
function personData(id) {
    if (id == "null") {
        if (valPer("null") == 1) {
            if (fotografia != null) {
                storageRef.child('images/personas/' + "'" + cloud[11] + "'").put(fotografia, metfoto).then(function(snapshot) {
        ...
        if (valPer(id) == 1) {
            if (fotografia != null) {
                ...
                db.ref('Personas').orderByKey().equalTo(cloud[11]).on("child_added", function(snapshot) {
                    cloud[10] = snapshot.val().Fotografia;
                    suRe("view");
                });
            }
        } else {
...
$('#eval').click(function() {
    ...
    dust.render('eval', {
        ...
        if (error) {
            ...
            $('#modal1').append(output);
            $('select').material_select();
            db.ref('Personas').orderByKey().equalTo('-KcWWmBeMAQMVbQ1yiVO').on("child_added", function(snapshot) {
                var name = snapshot.val().ApPat + ", " + snapshot.val().Nombre;
                var img = snapshot.val().Fotografia;
...
$('#pendiente').click(function() {
    ...
    dust.render('pendiente', {
        ...
        if (error) {
            ...
            empSel("null");
            $('#emp').change(function() {
                var li = document.getElementById('pendientes').querySelectorAll('LI');
                for (var i = 1; i < li.length; i++) {
                    document.getElementById('pendientes').removeChild(li[i])
                }
                db.ref('Pendientes').orderByChild('Empresa').equalTo($('#emp').val()).on("child_added", function(snapshot) {
                    if (snapshot != null) {
                        var new_li = document.createElement('LI');
                        new_li.id = "it+" + snapshot.key;
                        $(new_li).addClass("collection-item");
                        new_li.appendChild(document.createTextNode(snapshot.key));
                        document.getElementById('pendientes').appendChild(new_li);
                    }
                });
            });
            db.ref('Pendientes').on("child_removed", function(snapshot) {
                var ll = document.getElementById('it+' + snapshot.key);
                document.getElementById('pendientes').removeChild(ll);
            });
            $('#new').click(function() {
                var num = $('#number').val();
                var emp = $('#emp').val();
                if (emp != null && num > 0) {
                    for (var i = 0; i < num; i++) {
                        db.ref('Pendientes').push().child('Empresa').set(emp);
                    }
                    $('#number').val("");
                    Alert("Exito", "green");
                } else {
                    Alert("No uses información inválida", "red");
                }
            });
        }
    });
