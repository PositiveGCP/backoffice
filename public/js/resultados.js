...
function averaGe(key) {
    ...
    db.ref('Transfer').on("child_added", function(snapshot) {
        if (snapshot.val().key_empresa == key) {
            ...
            prom = res / i;
            list.appendChild(document.createTextNode(Math.round(prom))); // TODO: Aquí modifico Dante
        }
    });
}
...
function shoTrnsfr(id) {
    db.ref('Transfer').on("child_added", function(snapshot) {
        if (snapshot.val().key_empresa == id) {
            db.ref('Cuenta').orderByKey().equalTo(snapshot.val().key_empresa).on("child_added", function(snap) {
                db.ref('Usuarios').orderByKey().equalTo(snapshot.val().key_usuario).on("child_added", function(snp) {
                    db.ref('Personas').orderByKey().equalTo(snapshot.val().key_persona).on("child_added", function(sn) {
                        var res
                        if (snapshot.val().status == 'pendiente') {
                            res = 'Esperando...'
                        } else {
...
function genPDF() {
    html2canvas(document.body, {
        onrendered: function(canvas) {
            ...
            doc.save("Reporte.pdf"); /*NOMBRE DEL PDF*/
        }
    });
...
function reporte(id) {
    // db.ref('Personas').orderByKey().equalTo(id).on("child_added", function(snapshot) {
    //     db.ref('Cuenta').orderByKey().equalTo(snapshot.val().Empresa).on("child_added", function(snap) {
    //         array = {
    //             foto: snapshot.val().Fotografia,
    //             nombre: snapshot.val().ApPat + ", " + snapshot.val().Nombre,
    //             id: snapshot.key,
    //             puesto_a_aplicar: snapshot.val().Puesto,
    //             fecha_de_alta: "",
    //             correo: snapshot.val().email,
    //             telefono: snapshot.val().Telefono,
    //             sexo: snapshot.val().Genero,
    //             edad: "",
    //             tiempo_de_ingreso: "",
    //             logo: snap.val().Logotipo,
    //             empresa: snap.val().NComercial,
    //             rfc: ""
    //         };
    //         dust.render('pdfdemo', {
    //             array
